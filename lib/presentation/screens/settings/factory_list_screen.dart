/// ============================================================================
/// ফাইল: lib/presentation/screens/settings/factory_list_screen.dart
...
/// প্রধান অংশ: FactoryListScreen, _FactoryListScreenState, _AdminOnlyMessage
...
class _FactoryListScreenState extends State<FactoryListScreen> {
  ...
  Widget _content(BusinessSettingsEntity settings) => ListView(
    ...
    children: [
      ...
      if (settings.factoryList.isEmpty)
        const Card(
          child: Padding(
            padding: EdgeInsets.all(24),
            child: Center(child: Text('No factories added yet.')),
          ),
        )
      else
        Card(
          child: Column(
            children: [
              for (final factory in settings.factoryList) ...[
                ListTile(
                  leading: const Icon(
                  ...
                  trailing: IconButton(
                    ...
                    onPressed: () => _confirmDelete(settings, factory),
                  ),
                ),
                if (factory != settings.factoryList.last)
  ...
  void _add(BusinessSettingsEntity settings) {
    ...
    if (factory.isEmpty) {
      _message('Enter a factory name first.');
      return;
    }
    final exists = settings.factoryList.any(
      (item) => item.toLowerCase() == factory.toLowerCase(),
    );
    if (exists) {
      _message('This factory is already in the list.');
      return;
    }
    context.read<SettingsBloc>().add(
      UpdateFactoryList([...settings.factoryList, factory]),
    );
  ...
  Future<void> _confirmDelete(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: const Text('Remove factory?'),
        content: Text('Remove "$factory" from the factory list?'),
        actions: [
          TextButton(
    ...
    context.read<SettingsBloc>().add(
      UpdateFactoryList(
        settings.factoryList.where((item) => item != factory).toList(),
      ),
    );
  }
...
class _AdminOnlyMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Center(
    child: Padding(
      padding: EdgeInsets.all(24),
      child: Text(
        'Only administrators can manage factories.',
