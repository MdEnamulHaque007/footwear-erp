/// ============================================================================
/// ফাইল: lib/presentation/screens/production/production_detail_screen.dart
...
/// প্রধান অংশ: ProductionDetailScreen, _ProductionDetailScreenState
...
/// Production detail view: production information, PO information, quantity &
/// value summary, related entries for the same PO line and sync status.
class ProductionDetailScreen extends StatefulWidget {
  const ProductionDetailScreen({super.key, required this.id, this.initialItem});
  final String id;
...
class _ProductionDetailScreenState extends State<ProductionDetailScreen> {
  late final ProductionBloc _bloc;

  @override
  void initState() {
    super.initState();
    _bloc = GetIt.I<ProductionBloc>();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _bloc.add(
        LoadProductionDetail(widget.id, initialItem: widget.initialItem),
      );
    });
  }
    ...
    child: Scaffold(
      ...
      body: BlocBuilder<ProductionBloc, ProductionState>(
        builder: (context, state) {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    OutlinedButton(
                      onPressed: () =>
                          _bloc.add(LoadProductionDetail(widget.id)),
          ...
          if (state is! ProductionDetailLoaded) {
            return const Center(child: CircularProgressIndicator());
          }
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                _section('Production Information', [
                  _row('SL', '${item.sl}'),
                  _row('Production Date', _date(item.productionDate)),
                ...
                _section('Quantity & Value', [
                  _row('Sewing Quantity', '${state.sewingQuantity}'),
                  _row(
                    'Total Production Qty',
                    '${state.totalProductionQuantity}',
                  ),
                  _row('Available Quantity', '${state.availableQuantity}'),
                  _row('This Production Qty', '${item.quantity}'),
                  _row(
                    'Remaining After This',
                    '${state.availableQuantity - item.quantity}',
                  ),
                  _row('Unit Price', _money(item.unitPrice)),
                  _row('Production Value', _money(item.productionValue)),
                ]),
  ...
  Widget _relatedTable(ProductionDetailLoaded state) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
          ...
          rows: state.related.asMap().entries.map((entry) {
            ...
            return DataRow(
              cells: [
                ...
                DataCell(Text(_date(related.productionDate))),
                DataCell(Text(related.voucherNo)),
                DataCell(Text('${related.quantity}')),
                DataCell(Text(_money(related.productionValue))),
