/// ============================================================================
/// ফাইল: lib/presentation/screens/settings/brand_list_screen.dart
...
/// প্রধান অংশ: BrandListScreen, _BrandListScreenState, _AdminOnlyMessage
...
class _BrandListScreenState extends State<BrandListScreen> {
  ...
  Widget _content(BusinessSettingsEntity settings) => ListView(
    ...
    children: [
      ...
      if (settings.brandList.isEmpty)
        const Card(
          child: Padding(
            padding: EdgeInsets.all(24),
            child: Center(child: Text('No brands added yet.')),
          ),
        )
      else
        Card(
          child: Column(
            children: [
              for (
                var index = 0;
                index < settings.brandList.length;
                index++
              ) ...[
                ListTile(
                  leading: const Icon(
                  ...
                  title: Text(settings.brandList[index]),
                  trailing: IconButton(
                    tooltip: 'Delete brand',
                    icon: const Icon(Icons.delete_outline),
                    color: ColorPalette.error,
                    onPressed: () =>
                        _confirmDelete(settings, settings.brandList[index]),
                  ),
                ),
                if (index < settings.brandList.length - 1)
  ...
  void _add(BusinessSettingsEntity settings) {
    ...
    if (brand.isEmpty) {
      _message('Enter a brand name first.');
      return;
    }
    final exists = settings.brandList.any(
      (item) => item.toLowerCase() == brand.toLowerCase(),
    );
    if (exists) {
      _message('This brand is already in the list.');
      return;
    }
    context.read<SettingsBloc>().add(
      UpdateBrandList([...settings.brandList, brand]),
    );
  ...
  Future<void> _confirmDelete(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text('Remove "$brand" from the brand list?'),
        actions: [
          TextButton(
    ...
    context.read<SettingsBloc>().add(
      UpdateBrandList(
        settings.brandList.where((item) => item != brand).toList(),
      ),
    );
  }
...
class _AdminOnlyMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Center(
    child: Padding(
      padding: EdgeInsets.all(24),
      child: Text(
        'Only administrators can manage brands.',
